...
The GO enrichment is done in the `osgo` package. Load that and use the `do_enrich()` function, passing it the vector of `gene_ids` to calculate the enrichment. **Note that you must here specify which _O.sativa_ group you use.** Use the `which` argument with either `"indica"` or `"japonica"`. Choose the type of information you prefer to use as labels: `"go"` for the GO:number, `"name"` for the GO name, or `"description"` for the long description. 
...
To display the GO names:

```{r}
enrich_names <- do_enrich(gene_ids, which = "japonica", label_type = "name")
barplot(enrich_names, showCategory = 5)
```
